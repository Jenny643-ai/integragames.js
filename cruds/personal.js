...
router.get('/', (req, res) => {
    ...
    connection.query(sql, (err, result) => {
        ...
        result.forEach(row => {
            ...
            const rutaImagen = path.join(__dirname, '../img/responsables/', row.imagen || '');

            const imagen = (row.imagen && fs.existsSync(rutaImagen))
...
router.post('/crear', upload.single('imagen'), (req, res) => {
    ...
    connection.query(sql, [nombre, correo, contrasena, rol, imagenNombre], (err) => {
        if (err) throw err;
...
router.post('/editar', upload.single('imagen'), (req, res) => {
    ...
    connection.query(
        ...
        (err, result) => {
            ...
            let imagenNombre = result.length > 0
                ? result[0].imagen
                : 'sinFoto.jpg';
